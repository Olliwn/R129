...
*   **Instrument Cluster — Additional Faults (further evidence of swap/age):**
    ...
    * **Temperature LCD delaminated/washed out:** The small LCD panel that displays outside temperature is fading — segments are losing contrast and becoming difficult to read. This is a known R129 age issue caused by UV degradation and heat cycling of the LCD polarizer film. Fix options: (a) replace the LCD module, (b) re-polarize with new film (common DIY), or (c) source a better-condition used cluster. Both faults will be addressed during the Phase 3 cluster pull.
